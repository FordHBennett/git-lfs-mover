...
push_small_commits_to_branch()
{
    ...
    while read -r line; do
      commit_array+=("$line")
    done < <(git rev-list --reverse $hash)
    for ((i=0;i<${#commit_array[@]}; i+=1)); do
        object_size=$(git cat-file -s ${commit_array[i]})
        hash_size=$((hash_size + object_size))
...
main()
{
    ...
    refs_array=($refs)
    hashes=$(cat .git/packed-refs | awk '$2 ~ /^refs\/heads/{print $1}')
    hashes_array=($hashes)

    for((i=0; i<${#refs_array[@]}; i++)); do
      if [[ ! "${refs_array[i]}" =~ "HEAD" ]]; then
        local_branch=${refs_array[i]#refs/remotes/origin/} # remove the remote prefix
